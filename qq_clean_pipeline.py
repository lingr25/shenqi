...
def build_windows(
    ...
    for cid, idxs in enumerate(clusters):
        ...
        for lo, hi in merge_intervals(centers):
            ...
            source_ids = [r["msg_id"] for r in recs]
            auths = []
            seen_sid = set()
            fast = False
            term_set: list[str] = []
            ...
            for r in recs:
                if "mixed_media" in r["flags"]:
                    n_mixed += 1
                blob = r.get("text_stripped") or r["text"] or ""
                ...
                if r["qq"] in authority_qqs:
                    ...
                    auth_blob = r.get("text_stripped") or r["text"] or ""
                    if NUM_UNIT_RE.search(auth_blob) or UNPACK_RE.search(auth_blob):
                        fast = True
            ...
            windows.append(
                {
                    ...
                    "start_ts": recs[0]["ts"],
                    "end_ts": recs[-1]["ts"],
                    "start_time": recs[0]["time_str"],
                    "end_time": recs[-1]["time_str"],
                    "n_messages": len(recs),
                    "authorities": auths,
                    "hit_terms": term_set,
                    "score": score,
...
def write_report(
    ...
    a("")
    a(f"- 窗总数：{len(windows)}")
    a(f"- tier `candidate`：{tiers.get('candidate', 0)}")
    a(f"- tier `maybe`：{tiers.get('maybe', 0)}")
    a(f"- tier `ignore`：{tiers.get('ignore', 0)}")
    ...
    for i, w in enumerate(top, 1):
        ...
        a(
            ...
            f"n={w['n_messages']} score={w['score']} tier={w['tier']} "
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="QQ 群聊清洗流水线（第 0–3 步）")
    ...
    p.add_argument("--seed", type=int, default=42, help="ignore 抽查随机种子")
