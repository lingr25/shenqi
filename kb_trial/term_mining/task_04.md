...
- 逐字稿：`transcripts_txt/BV14y8269EbS_p1_41164079923.txt`（行首 `[HH:MM:SS]` 时间戳）
...
写到 `kb_trial/term_mining/out/out_04.jsonl`（`out/` 不存在就自己创建），UTF-8 JSONL，每条一行：

```json
{"file":"BV14y8269EbS_p1_41164079923","kind":"asr_suspect|unknown_entity|slang","term":"原文字符串","correction":"推断正词(没有就null)","ts":"行首时间戳","context":"该句原文(<=80字)","confidence":"high|mid|low","note":"一句话理由"}
```
...
- 输入：`transcripts_txt/BV14y8269EbS_p1_41164079923.txt`
- 输出：`kb_trial/term_mining/out/out_04.jsonl`
