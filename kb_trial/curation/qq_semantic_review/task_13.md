...
读取批次文件：`kb_trial/curation/qq_semantic_review/batch_13.jsonl`（13 见本任务末尾），每行一条 JSON：
...
写到 `kb_trial/curation/qq_semantic_review/out/out_13.jsonl`（`out/` 不存在就自己创建）。UTF-8 JSONL，每条输入**恰好一行**，保持输入顺序：
...
- 输入：`kb_trial/curation/qq_semantic_review/batch_13.jsonl`
- 输出：`kb_trial/curation/qq_semantic_review/out/out_13.jsonl`
