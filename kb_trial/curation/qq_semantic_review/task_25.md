...
读取批次文件：`kb_trial/curation/qq_semantic_review/batch_25.jsonl`（25 见本任务末尾），每行一条 JSON：
...
- 注意：你的内置知识库可能偏旧，遇到没见过的新干员/新机制名词，**先查 glossary.json；查不到也不许凭印象判错**，在 issues 里标 `unknown_term:<词>` 交给人工
...
写到 `kb_trial/curation/qq_semantic_review/out/out_25.jsonl`（`out/` 不存在就自己创建）。UTF-8 JSONL，每条输入**恰好一行**，保持输入顺序：
...
- 输入：`kb_trial/curation/qq_semantic_review/batch_25.jsonl`
- 输出：`kb_trial/curation/qq_semantic_review/out/out_25.jsonl`
